...
##
    ...
    for data,idx in train_FMNIST_loader :
      if len(X)>10000:
        break
      if len(X) ==0:
        X = model_FMNIST.for_at(data)
        Y = idx
      else :
        X = torch.cat((X,model_FMNIST.for_at(data)))
        Y = torch.cat((Y,idx))
    ...
    X_check = torch.cat([X,model_FMNIST.for_at(fin_img_list)],dim = 0)
    print(X_check.shape)
    ...
    for data,idx in train_MNIST_loader :
      if len(X)>5000:
        break
      if len(X) ==0:
        X = model_MNIST.for_at(data)
        Y = idx
      else :
        X = torch.cat((X,model_MNIST.for_at(data)))
        Y = torch.cat((Y,idx))
    ...
    tsne = TSNE(n_iter=400)
    X_check = torch.cat([X,model_MNIST.for_at(fin_img_list.reshape(-1,1,28,28))],dim = 0)
    print(X_check.shape)
    tsne_results = tsne.fit_transform(X_check.detach().numpy())
    Y_check = torch.cat([Y*0, fin_label_list])
    # Create the figure
